...
resource "aws_lb" "elohor_alb" {
  name               = "elohor123abcalb"
  internal           = false
...
resource "aws_lb_listener" "elohor_alb_listner" {
  load_balancer_arn = aws_lb.elohor_alb.arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.elohor_alb_tg.arn
  }
}

# create a target group for the loadbalancer
resource "aws_lb_target_group" "elohor_alb_tg" {
  name     = "elohoralbtg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = var.vpc_id
